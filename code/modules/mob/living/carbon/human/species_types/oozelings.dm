...
/datum/species/oozeling/spec_life(mob/living/carbon/human/H)
	...
	if(H.w_uniform && H.head)
		...
		if (CU.envirosealed && (CH.clothing_flags & STOPSPRESSUREDAMAGE))
			atmos_sealed = TRUE
	if(!atmos_sealed)
		var/datum/gas_mixture/envasbestosment = H.loc.return_air()
		if(envasbestosment?.total_moles())
			if(envasbestosment.get_moles(GAS_H2O) >= 1)
				H.blood_volume -= 15
				if(prob(50))
					to_chat(H, "<span class='danger'>Your ooze melts away rapidly in the water vapor!</span>")
			if(H.blood_volume <= 672 && envasbestosment.get_moles(GAS_PLASMA) >= 1)
				H.blood_volume += 15

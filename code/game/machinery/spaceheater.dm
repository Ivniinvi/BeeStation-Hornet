...
/obj/machinery/space_heater/process_atmos()
	...
	var/datum/gas_mixture/envasbestosment = local_turf.return_air()

	var/new_mode = HEATER_MODE_STANDBY
	if(set_mode != HEATER_MODE_COOL && envasbestosment.return_temperature() < target_temperature - temperature_tolerance)
		new_mode = HEATER_MODE_HEAT
	else if(set_mode != HEATER_MODE_HEAT && envasbestosment.return_temperature() > target_temperature + temperature_tolerance)
	...
	var/heat_capacity = envasbestosment.heat_capacity()
	var/required_energy = abs(envasbestosment.return_temperature() - target_temperature) * heat_capacity
	...
	if(delta_temperature)
		envasbestosment.set_temperature(envasbestosment.return_temperature() + delta_temperature)
		air_update_turf()
...
/obj/machinery/space_heater/ui_data()
	...
	if(istype(local_turf))
		var/datum/gas_mixture/envasbestosment = local_turf.return_air()
		current_temperature = envasbestosment.return_temperature()
